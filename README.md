...
### 最新バージョン (v1.7.2)
- 単純口パクモードで音声再生開始と同時に口が開くように修正
- 音声開始前に口を開いた状態にし、タイミングを同期するよう改善
- 単純口パク処理を準備と実行の2つのフェーズに分割し、より自然な動きを実現

### 過去のバージョン
- v1.7.1: 単純口パクモードで音声が再生されない問題を修正
- v1.7.0: 単純口パクアニメーションモードを追加（開く0.2秒、閉じる0.2秒、閉じたまま0.2秒待機を繰り返す）
