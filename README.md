...
キャラクターの口の開き具合は、`client/main.js`の`applyMouthOpenValue`関数内で調整できます。v1.4では3.5倍に増幅されています。

```javascript
// 口の開きを大きくするために値を増幅（3.5倍に増幅）
...
最新のアップデート（v1.4）では、リップシンク機能に以下の改善を加えました：
...
これらの改善により、キャラクターの口の動きがより自然で、はきはきとした印象になり、実際の人間の話し方に近い動きを実現しています。
